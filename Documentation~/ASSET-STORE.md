...
2. Create a product draft and **reserve the technical name `dev.polyfork.unity`** — the
...
- [ ] **Reserve the technical name `dev.polyfork.unity`** in the product draft. The uploader
...
- [ ] Unzip `Polyfork-UPM.zip` into a fresh Unity 6000.0 project's `Packages/`, so the manifest
      lands at `Packages/dev.polyfork.unity/package.json`. Package Manager resolves glTFast
